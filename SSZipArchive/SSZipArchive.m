...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    return [SSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password ignoreFilesNamed:filenames];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    SSZipArchive *zipArchive = [[SSZipArchive alloc] initWithPath:path];
    NSMutableArray<NSString*> * ignoreFilenames = [[NSMutableArray alloc] init];
    if (filenames != nil)
        [ignoreFilenames addObjectsFromArray: filenames];
    ...
    if (success) {
        ...
        for (fileName in allObjects) {
            ...
            {
                if ([[NSFileManager defaultManager] subpathsOfDirectoryAtPath:fullFilePath error:nil].count == 0 && !shouldSkip)
                {
